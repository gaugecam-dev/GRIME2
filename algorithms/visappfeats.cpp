...
GC_STATUS VisAppFeats::Calculate( const FeatCalcItem &item, FeatureSet &feats )
{
    GC_STATUS retVal = GC_OK;
    try
    {
        if ( 0 > featCalcParams.timeStampStartPos || 0 > featCalcParams.timeStampLength ||
             featCalcParams.areaROIs.empty() || featCalcParams.areaROIs.empty() )
        {
            FILE_LOG( logERROR ) << "[VisAppFeats::Calculate] Feature calculation parameters not initialized";
            retVal = GC_ERR;
        }
        else
        {
            ...
            {
                GcTimestamp gcStamp;
                string stringWithTime;
                bool extractTimeFromString = true;
                if ( FROM_FILENAME == featCalcParams.timeStampType )
                {
                    stringWithTime = fs::path( item.imagePath ).filename().string();
                }
                else if ( FROM_EXIF == featCalcParams.timeStampType )
                {
                    stringWithTime = feats.exif.captureTime;
                }
                else
                {
                    extractTimeFromString = false;
                }

                if ( extractTimeFromString )
                {
                    retVal = GcTimestampConvert::GetGcTimestampFromString( stringWithTime, featCalcParams.timeStampStartPos,
                                                                           featCalcParams.timeStampLength, featCalcParams.timeStampFormat, gcStamp );
                    if ( GC_OK == retVal )
                    {
                        feats.imgTimestamp = GcTimestampConvert::GetISOTimestampFromGcTimestamp( gcStamp );
                    }
                }
                if ( GC_OK == retVal )
                {
                    Mat img = imread( item.imagePath, IMREAD_COLOR );
                    if ( img.empty() )
                    {
                        FILE_LOG( logERROR ) << "[VisAppFeats::Calculate] Could not open image: " << item.imagePath;
                        retVal = GC_ERR;
                    }
                    else
                    {
                        double angle;
                        Point offset;
                        Mat gray, mask, scratch, scratchAdj;

                        cvtColor( img, gray, COLOR_BGR2GRAY );
                        retVal = anchor.Find( gray, angle, offset );
                        if ( GC_OK == retVal )
                        {
                            Mat rotMat = getRotationMatrix2D( Point2f( img.cols >> 1, img.rows >> 1 ), angle, 1.0 );
                            warpAffine( img, scratch, rotMat, img.size() );
                            Mat transMat = ( Mat_< double >( 2, 3 ) << 1, 0, offset.x, 0, 1, offset.y );
                            warpAffine( scratch, scratchAdj, transMat, img.size() );
                        }
                    }
                }
            }
    ...
    {
        FILE_LOG( logERROR ) << "[VisAppFeats::Calculate] " << diagnostic_information( e );
    }

    return retVal;
}
GC_STATUS VisAppFeats::WriteFeatSetToCSV( const string filepath, const FeatureSet &featSet )
{
    ...
    if ( GC_OK == retVal )
    {
        try
        {
            ...
            if ( !outFile.is_open() )
            {
                FILE_LOG( logERROR ) << "[VisAppFeats::WriteFeatSetToCSV] Could not open CSV: " << filepath;
                retVal = GC_ERR;
            }
            else
            {
                ...
                outFile << featSet.imageSize.width << ",";
                outFile << featSet.imageSize.height << ",";
        ...
        {
            FILE_LOG( logERROR ) << "[VisAppFeats::WriteFeatSetToCSV] " << diagnostic_information( e );
            retVal = GC_EXCEPT;
        }
    }
    return retVal;
